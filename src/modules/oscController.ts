...
const RECONNECT_COOLDOWN = 10000;
...
export function createClient(instance: RCVInstance, ipAddress: string, port: number = 10024): void {
	...
	connectTimeoutTimer = setTimeout(() => {
		const err: NodeJS.ErrnoException = new Error(`connect ETIMEDOUT ${ipAddress}:${port}`);
		err.code = 'ETIMEDOUT';
	...
	onDataHandler = async (data) => {
		try {
			...
			while (buffer.length >= 4) {
				// Ensure there are at least 4 bytes to read the message size
					...
					if ('address' in parsedPacket && parsedPacket.address.startsWith('/')) {
						...
						for (const element of parsedPacket.packets) {
							if ('address' in element && element.address.startsWith('/')) {
								...
								ConsoleLog(
									instance,
									`Bundle element message: ${element.address}, args: ${JSON.stringify(elementValues)}`,
									LogLevel.INFO,
									false,
								);
							}
	...
	onCloseHandler = async (hadError) => {
		...
		ConsoleLog(
			instance,
			`Attempting to reconnect in ${RECONNECT_COOLDOWN / 1000}s... (${reconnectionAttempts})`,
			LogLevel.INFO,
			false,
		);
	...
	socket.connect(port, ipAddress, async () => {
		...
		if (instance.globalSettings.selectedDevice && instance.globalSettings.selectedDevice.name.startsWith('[Offline]')) {
			instance.globalSettings.selectedDevice.name = instance.globalSettings.selectedDevice.name.replace(
				'[Offline] ',
				'',
			);
		}
