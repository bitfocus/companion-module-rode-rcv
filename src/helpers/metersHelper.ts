		...
		for (let i = 0; i < MetersChannel.NumSources; i++) {
			if ((outputsMask & (BigInt(1) << BigInt(i))) !== BigInt(0)) {
				if (blob.length < 16) {
					ConsoleLog(
						instance,
						`Blob is too short to read output channel at index ${i}. Length: ${blob.length}`,
						LogLevel.DEBUG,
						false,
					);
					continue;
		...
		for (let i = 0; i < MetersChannel.NumSources; i++) {
			if ((inputsMask & (BigInt(1) << BigInt(i))) !== BigInt(0)) {
				if (blob.length < 16) {
					ConsoleLog(
						instance,
						`Blob is too short to read input channel at index ${i}. Length: ${blob.length}`,
						LogLevel.DEBUG,
						false,
					);
					continue;
...
export function getAllChannels(): MetersChannel[] {
	return Object.values(MetersChannel).filter(
		(value): value is number => typeof value === 'number' && value >= 0 && value < MetersChannel.NumSources,
	);
}
...
export const companionMeter = (
	options: PresetOptionsMeter1,
	orientation: 'horizontal' | 'vertical' = 'vertical',
	position: 'tl' | 'tr' | 'tm' | 'bl' | 'br' | 'bm' = 'br',
): Uint8Array => {
