...
import { FeedbackId, UpdateFeedbacks } from '../feedbacks/feedbacks.js';
import { setSyncDevice } from '../helpers/connectionHelpers.js';
...
export async function handleIncomingData(instance: RCVInstance, command: string, args: any) {
	...
	if (command === '/show') {
		...
		if (parsed) {
			...
			if (
				parsed.RcvShow &&
				(parsed.RcvShow['@_transition'] || parsed.RcvShow['@_transition_data']) &&
				parsed.RcvShow['@_transition_time'] !== undefined &&
				parsed.RcvShow['@_transition_time'] !== null
			) {
			...
			if (parsed.RcvShow && parsed.RcvShow.AudioMixer && parsed.RcvShow.AudioMixer.audiosources) {
				...
				const idToSubmixChannel = Object.fromEntries(Object.entries(submixList).map(([key, value]) => [value.id, key]));
					...
					if (audiosources[mix_key]['enabled'] === true) {
						if (
							...
							if (instance.variables.some((variable) => variable.variableId === varName)) {
								...
								ConsoleLog(
									instance,
									`Setting variable ${varName} to value ${audiosources[mix_key]['level']}`,
									LogLevel.DEBUG,
									false,
								);
							} else {
								...
								ConsoleLog(
									instance,
									`Creating variable ${varName} with value ${audiosources[mix_key]['level']}`,
									LogLevel.DEBUG,
									false,
								);
							}
							...
							ConsoleLog(
								instance,
								`Mixer channel ${audiosources[mix_key]['name']} (${mix_key}) updated with gain value ${audiosources[mix_key]['level']}.`,
								LogLevel.DEBUG,
								false,
							);
						}
						...
						for (const [mix] of Object.entries(mixes)) {
							...
							if (match) {
								...
								if (instance.variables.some((variable) => variable.variableId === varName)) {
									...
									ConsoleLog(
										instance,
										`Setting variable ${varName} to value ${floatToDb(submixData.level)}. Muted: ${submixData.muted}`,
										LogLevel.DEBUG,
										false,
									);
								} else {
									if (submix !== undefined) {
										...
										ConsoleLog(
											instance,
											`Creating variable ${varName} with value ${floatToDb(submixData.level)}`,
											LogLevel.DEBUG,
											false,
										);
									}
								...
								ConsoleLog(
									instance,
									`[2] Submix ${submix} changed ${mixerChannels[mix_key].name} (${mix_key}) LEVEL to ${submixData.level}.`,
									LogLevel.DEBUG,
									false,
								);
							}
						...
						if (mixerChannels.hasOwnProperty(mix_key)) {
							...
							ConsoleLog(
								instance,
								`Removed Mixer channel ${audiosources[mix_key]['name']} (${mix_key}).`,
								LogLevel.DEBUG,
								false,
							);
						}
			...
			if (parsed.RcvShow && parsed.RcvShow.VideoInputs) {
				...
				if (Array.isArray(_videoSources)) {
					...
					_videoSources.forEach((source, index) => {
						...
						ConsoleLog(
							instance,
							`[XML 1] ${index} Video source (${source['name']}) updated with source type ${source['source']}.`,
							LogLevel.DEBUG,
							false,
						);
					});
					...
					Object.entries(_videoSources).forEach(([index, source]) => {
						...
						ConsoleLog(
							instance,
							`[XML 2] ${numericIndex} Video source (${source['name']}) updated with source type ${source['source']}.`,
							LogLevel.DEBUG,
							false,
						);
					});
			...
			if (parsed.RcvShow && parsed.RcvShow.Overlays) {
				...
				if (Array.isArray(_overlaySources)) {
					...
					Object.entries(_overlaySources).forEach(([index, source]) => {
						...
						ConsoleLog(
							instance,
							`[XML 2] ${numericIndex} Overlay source (${source['name']}) updated.`,
							LogLevel.DEBUG,
							false,
						);
					});
			...
			if (
				parsed.RcvShow &&
				parsed.RcvShow['@_PgmOverlay'] !== null &&
				parsed.RcvShow['@_PvwOverlay'] !== null &&
				parsed.RcvShow['@_PgmOverlay'] !== undefined &&
				parsed.RcvShow['@_PvwOverlay'] !== undefined
			) {
				...
				for (const overlay in overlaySources) {
					if (overlaySources.hasOwnProperty(overlay)) {
						...
						if (overlayNumber === _pgmOverlay) {
							...
							if (
								overlaySources[overlayNumber].name === null ||
								overlaySources[overlayNumber].name === undefined ||
								overlaySources[overlayNumber].name === ''
							) {
			...
			if (
				parsed.RcvShow &&
				parsed.RcvShow['@_PgmScene'] !== null &&
				parsed.RcvShow['@_PvwScene'] !== null &&
				parsed.RcvShow['@_PgmScene'] !== undefined &&
				parsed.RcvShow['@_PvwScene'] !== undefined
			) {
				...
				for (const scene in sceneSources) {
					if (sceneSources.hasOwnProperty(scene)) {
						...
						if (sceneNumber === _PgmScene) {
							...
							if (
								sceneSources[sceneNumber].name === null ||
								sceneSources[sceneNumber].name === undefined ||
								sceneSources[sceneNumber].name === ''
							) {
			...
			if (debug) {
				console.dir(parsed.RcvShow, { depth: null, colors: false });
			}
		}
	...
	if (command.includes('/show/switchingMode')) {
		...
		instance.checkFeedbacks(FeedbackId.switching_mode);
	...
	if (command.includes('/device/buttons/108/colour')) {
		...
		switch (args[0]) {
			case 11:
				buttonList[buttonPressControlType.BUTTON_INSPECT].state = 2; //Lit
				break;
			default:
				buttonList[buttonPressControlType.BUTTON_INSPECT].state = 3; //Off
				break;
		}
		...
		for (const source in sources) {
			if (sources.hasOwnProperty(source)) {
				...
				if (sourceNumber === inputId) {
					...
					sources[sourceNumber].state =
						sources[sourceNumber].name === null ||
						sources[sourceNumber].name === undefined ||
						sources[sourceNumber].name === ''
							? 3 // Not Assigned
							: 0; // Unselected
				...
				const buttonState = buttonList[sourceKey]?.state;

				if (sourceType === 'input' && buttonState !== undefined) {
					// Don't stomp if this input is currently PGM or PVW
					const isPgm = sourceNumber === controllerVariables.PgmInput;
					const isPvw = controllerVariables.studioMode && sourceNumber === controllerVariables.PvwInput;

					// Only apply button-derived state when the source is currently "unselected" (0)
					// and it is NOT the selected PGM/PVW input.
					if (sources[sourceNumber].state === 0 && !isPgm && !isPvw) {
						switch (buttonState) {
							case 1:
								sources[sourceNumber].state = 0;
								break; // button Idle -> source Idle
							case 2:
								sources[sourceNumber].state = 1;
								break; // button Active -> source Active
							case 3:
								sources[sourceNumber].state = 2;
								break; // button Preview -> source Preview
							case 0:
							default:
								sources[sourceNumber].state = 3;
								break; // button Invalid -> source Invalid
						}
					}
				}
			}
	...
	if (sceneButtonsFilter.test(command)) {
		...
		ConsoleLog(instance, `Button ${key[3]} set to ${args[0]}`, LogLevel.DEBUG, false);
		instance.checkFeedbacks(FeedbackId.visualSwitcher);

		const allButtonPressTypes = [...Object.values(buttonPressControlType)];

		for (const button of allButtonPressTypes) {
			const buttonProperties = buttonList[button as keyof typeof buttonList];

			// 02 - Unassigned, 03 - Standby, 04 - PGM, 05 - PV, 07 - MediaBus, 10 - Inspect, 11 - Inspect Active, 8 - Overlay, 13 - Invalid
			...
			if (buttonProperties) {
				if (buttonProperties.path && buttonProperties.path === command) {
					...
					const match = /^button([1-6])$/.exec(String(button));
					if (match) {
						const pairedKey = `input${match[1]}`;

						if (buttonList[pairedKey]) {
							// Only set idle/unavailable here. Let PGM/PVW come from /show/*current.
							if (args[0] === 2 || args[0] === 13) {
								buttonList[pairedKey].state = 0; // button Invalid
							} else if (args[0] === 3 || args[0] === 7 || args[0] === 8 || args[0] === 10 || args[0] === 11) {
								buttonList[pairedKey].state = 1; // button Idle
							}
							// args[0] === 4 (PGM) or 5 (PVW): do nothing here on purpose
						}
					}

					ConsoleLog(
						instance,
						`Button ${buttonProperties.title} state set to ${buttonProperties.state}. Exec: ${button as keyof typeof buttonList}`,
						LogLevel.DEBUG,
						false,
					);
				}
	...
	if (command === '/show/transition_data') {
		...
		if (
			controllerVariables.currentTransitionCat === transtionCategory.WIPE ||
			controllerVariables.currentTransitionCat === transtionCategory.DIP
		) {
		...
		const updateSubmixProperty = (mixerChNo: number, property: string, value: any) => {
			...
			if (submixList.hasOwnProperty(_submix)) {
				...
				if (submixIndex !== undefined && submixIndex >= 0) {
					...
					const newSubmix: AudioAudioSourceCh = {
						submix: _submix as SubmixChannels,
						level: 0,
						muted: false,
						disabled: false,
						linked: false,
				...
				ConsoleLog(
					instance,
					`${mixerChannels[mixerChName].name} (value${mixerCh}) changed on Submix ${_submix} ${property.toUpperCase()} to ${value}.`,
					LogLevel.DEBUG,
					false,
				);
			}
	...
	if (command.includes('/audioSource')) {
		...
		if (property === 'level') {
			if (mixerChannels.hasOwnProperty(mixerCh)) {
				...
				ConsoleLog(
					instance,
					`Mixer channel ${mixerChannels[mixerCh].name} (${mixerCh}) gain updated with value ${mixerChannels[mixerCh].gain}.`,
					LogLevel.DEBUG,
					false,
				);
			}
			...
			if (mixerChannels.hasOwnProperty(mixerCh)) {
				...
				ConsoleLog(
					instance,
					`Mixer channel ${mixerChannels[mixerCh].name} (${mixerCh}) updated with position ${mixerChannels[mixerCh].position}.`,
					LogLevel.DEBUG,
					false,
				);
			}
			...
			if (mixerChannels.hasOwnProperty(mixerCh)) {
				...
				ConsoleLog(
					instance,
					`Mixer channel ${oldName} (${mixerCh}) updated with name ${mixerChannels[mixerCh].name}.`,
					LogLevel.DEBUG,
					false,
				);
			}
			...
			if (mixerChannels.hasOwnProperty(mixerCh)) {
				...
				ConsoleLog(
					instance,
					`Mixer channel ${mixerChannels[mixerCh].name} (${mixerCh}) scene mute set to ${scene_mute}.`,
					LogLevel.DEBUG,
					true,
				);
			}
	...
	if (command.includes('/show/hdmiOut')) {
		...
		instance.checkFeedbacks(FeedbackId.routing);
	...
	if (command === '/menuSelected') {
		instance.setVariableValues({
			active_menu: args[0].toString(),
		});
				...
				if (outputData) {
					...
					if (!instance.variables.some((variable) => variable.variableId === varR)) {
						instance.variables.push({
							variableId: varR,
							name: `${mixerChannels[channelString].name} Live Right Level`,
						});
						instance.setVariableDefinitions(instance.variables);
	...
	if (command.includes('/audio_settings/active_rsync_device_pid')) {
		const key = command.split('/');
		const pid = args[0];
		let syncDeviceName = 'No Device';

		if (pid !== RCVSyncDevice.NoDevice) {
			controllerVariables.sync_active = true;
			controllerVariables.sync_device = pid;

			syncDeviceName = setSyncDevice(pid);

			ConsoleLog(instance, `SYNC Activated. Device: ${syncDeviceName} Pid: ${pid}`, LogLevel.DEBUG, true);
		} else {
			controllerVariables.sync_active = false;
			controllerVariables.sync_device = RCVSyncDevice.NoDevice;
			ConsoleLog(instance, `SYNC Deactivated.`, LogLevel.DEBUG, true);
		}

		instance.setVariableValues({
			sync_active: controllerVariables.sync_active.toString(),
			sync_device: syncDeviceName,
		});
